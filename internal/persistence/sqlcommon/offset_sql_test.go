...
func TestOffsetsE2EWithDB(t *testing.T) {
	...
	// Update the offset (this is testing what's possible at the persistence layer,
	...
	err = s.UpsertOffset(context.Background(), offsetUpdated)
	assert.NoError(t, err)
	...
	fb := persistence.OffsetQueryFactory.NewFilter(ctx, 0)
	filter := fb.And(
...
func TestUpsertOffsetFailBegin(t *testing.T) {
	...
	err := s.UpsertOffset(context.Background(), &fftypes.Offset{})
...
func TestUpsertOffsetFailSelect(t *testing.T) {
	...
	mock.ExpectRollback()
	err := s.UpsertOffset(context.Background(), &fftypes.Offset{Name: "name1"})
...
func TestGetOffsetQueryFail(t *testing.T) {
	s, mock := getMockDB()
	mock.ExpectQuery("SELECT .*").WillReturnError(fmt.Errorf("pop"))
	f := persistence.OffsetQueryFactory.NewFilter(context.Background(), 0).Eq("type", "")
...
func TestGetOffsetBuildQueryFail(t *testing.T) {
	s, _ := getMockDB()
	f := persistence.OffsetQueryFactory.NewFilter(context.Background(), 0).Eq("type", map[bool]bool{true: false})
	_, err := s.GetOffsets(context.Background(), f)
