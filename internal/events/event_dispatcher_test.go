...
func TestEventDispatcherBufferedDeliveryAcksReadahead(t *testing.T) {
	log.SetLevel("debug")
	var five = uint64(5)
	sub := &subscription{
		dispatcherElection: make(chan bool, 1),
		...
		eventMatcher: regexp.MustCompile(fmt.Sprintf("^%s$", fftypes.EventTypeMessageConfirmed)),
	}
	...
	go func() {
		repoll, err := ed.bufferedDelivery([]*fftypes.Event{
			...
			{ID: ev3, Sequence: 10000003, Reference: ref3, Type: fftypes.EventTypeDataArrivedBroadcast},
