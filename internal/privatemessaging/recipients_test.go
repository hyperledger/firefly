...
func TestResolveMemberListNewGroupE2E(t *testing.T) {
	...
	mdi.On("GetIdentities", pm.ctx, mock.Anything).Return([]*core.Identity{remoteNode}, nil, nil).Once()
...
func TestResolveMemberListExistingGroup(t *testing.T) {
	...
	mim.On("GetNodeOwnerOrg", pm.ctx, "ns1").Return(localNode, nil)
...
func TestResolveMemberNodeOwnedParentOrg(t *testing.T) {
	...
	mdi.On("GetGroupByHash", pm.ctx, mock.Anything, mock.Anything).Return(&core.Group{Hash: fftypes.NewRandB32()}, nil, nil).Once()
	mim := pm.identity.(*identitymanagermocks.Manager)
	mim.On("GetNodeOwnerOrg", pm.ctx, "ns1").Return(parentOrg, nil)
...
func TestResolveNodeByIDNoResult(t *testing.T) {
	...
	mdi := pm.database.(*databasemocks.Plugin)
	mdi.On("GetIdentities", pm.ctx, mock.Anything).Return([]*core.Identity{}, nil, nil)
...
func TestResolveReceipientListExisting(t *testing.T) {
	...
	mdi := pm.database.(*databasemocks.Plugin)
	mdi.On("GetGroupByHash", pm.ctx, groupID).Return(&core.Group{Hash: groupID}, nil)
