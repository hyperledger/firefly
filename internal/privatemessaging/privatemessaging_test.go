...
func TestDispatchBatchWithBlobs(t *testing.T) {
	...
	mdi.On("GetBlob", pm.ctx, mock.Anything, mock.Anything, blob1).Return(&core.Blob{
		Hash:       blob1,
		PayloadRef: "/blob/1",
	}, nil)
...
func TestWriteTransactionSubmitBatchPinFail(t *testing.T) {
	...
	mdi.On("GetBlob", pm.ctx, "ns1", mock.Anything, blob1).Return(&core.Blob{
		Hash:       blob1,
		PayloadRef: "/blob/1",
	}, nil)
...
func TestTransferBlobsNotFound(t *testing.T) {
	...
	mdi := pm.database.(*databasemocks.Plugin)
	mdi.On("GetBlob", pm.ctx, mock.Anything, mock.Anything, mock.Anything).Return(nil, nil)
...
func TestTransferBlobsOpInsertFail(t *testing.T) {
	...
	mdi.On("GetBlob", pm.ctx, mock.Anything, mock.Anything, mock.Anything).Return(&core.Blob{PayloadRef: "blob/1"}, nil)
