...
func GenerateObjectsReferenceMarkdown(ctx context.Context) (map[string][]byte, error) {
	...
	types := []interface{}{
		...
		&fftypes.Event{
			...
			Topic:       fftypes.TransactionTypeBatchPin.String(),
			Created:     fftypes.UnixTime(1652664195),
		},

		&fftypes.Subscription{
		...
		&fftypes.WSClientActionStartPayload{
		...
		&fftypes.WSClientActionAckPayload{
			WSClientActionBase: fftypes.WSClientActionBase{
		...
		&fftypes.WSProtocolErrorPayload{
