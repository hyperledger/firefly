...
func TestCheckGeneratedMarkdownPages(t *testing.T) {
	...
	for pageName, markdown := range markdownMap {
		...
		t.Log(string(b))
		assert.Equal(t, actualPageHash.Sum(nil), expectedPageHash.Sum(nil), "The type reference docs generated by the code did not match the docs files in git for page: '%s.md' Did you forget to run `make reference`?", pageName)
	}
}
