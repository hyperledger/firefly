...
func TestRegisterOrganizationChildOk(t *testing.T) {
	...
	mbm.On("BroadcastDefinition", nm.ctx, mock.Anything, parentID, "ff-org-0x12345", fftypes.SystemTopicDefineOrganization).Return(mockMsg, nil)
...
func TestRegisterNodeOrganizationRootOk(t *testing.T) {
	...
	mockMsg := &fftypes.Message{Header: fftypes.MessageHeader{ID: fftypes.NewUUID()}}
	mbm := nm.broadcast.(*broadcastmocks.Manager)
	mbm.On("BroadcastDefinition", nm.ctx, mock.Anything, rootID, "ff-org-0x12345", fftypes.SystemTopicDefineOrganization).Return(mockMsg, nil)
