...
var (
	...
	MsgDIDResovlerUnknown           = ffm("FF10346", "DID resolver unknown for DID: %s", 400)
	MsgIdentityNotOrg               = ffm("FF10347", "Identity '%s' with DID '%s' is not an organization", 400)
	MsgIdentityNotNode              = ffm("FF10348", "Identity '%s' with DID '%s' is not a node", 400)
	MsgBlockchainKeyNotSet          = ffm("FF10349", "No blockchain key specified", 400)
	MsgNoVerifierForIdentity        = ffm("FF10350", "No %s verifier registered for identity %s", 400)
	MsgNodeMissingBlockchainKey     = ffm("FF10351", "No organization signing key configured on node", 400)
	MsgAuthorRegistrationMismatch   = ffm("FF10352", "Verifier '%s' cannot be used for signing with author '%s'. Verifier registered to '%s'", 400)
	MsgAuthorMissingForKey          = ffm("FF10353", "Key '%s' has not been registered by any identity, and a separate 'author' was not supplied", 400)
	MsgAuthorIncorrectForRootReg    = ffm("FF10354", "Author namespace '%s' and DID '%s' combination invalid for root organization registration", 400)
	MsgKeyIdentityMissing           = ffm("FF10355", "Identity owner of key '%s' not found", 500)
	MsgCustomIdentitySystemNS       = ffm("FF10356", "Custom identities cannot be defined in the '%s' namespace", 400)
	MsgNilParentIdentity            = ffm("FF10357", "Identity of type '%s' must have a valid parent", 400)
	MsgSystemIdentityCustomNS       = ffm("FF10358", "System identities must be defined in the '%s' namespace", 400)
	MsgUnknownIdentityType          = ffm("FF10359", "Unknown identity type: %s", 400)
	MsgInvalidDIDForType            = ffm("FF10360", "Invalid FireFly DID '%s' for type='%s' namespace='%s' name='%s'", 400)
	MsgIdentityChainLoop            = ffm("FF10361", "Loop detected on identity %s in chain for %s (%s)", 400)
	MsgInvalidIdentityParentType    = ffm("FF10362", "Parent %s (%s) of type %s is invalid for child %s (%s) of type", 400)
	MsgParentIdentityMissingClaim   = ffm("FF10363", "Parent %s (%s) is invalid (missing claim)", 400)
)
