...
var (
	...
	MsgDupDataReferenceID           = ffe("FF10124", "Duplicate data ID in message '%s'")
	...
	MsgMaxFilterSkip                = ffe("FF10183", "You have reached the maximum pagination limit for this query (%d)")
	MsgMaxFilterLimit               = ffe("FF10184", "Your query exceeds the maximum filter limit (%d)")
	...
	MsgNodeAndOrgIDMustBeSet        = ffe("FF10216", "node.name, org.name and org.identity must be configured first", 409)
	MsgBlobStreamingFailed          = ffe("FF10217", "Blob streaming terminated with error", 500)
	MsgMultiPartFormReadError       = ffe("FF10218", "Error reading multi-part form input", 400)
	...
	MsgTokenPoolDuplicate           = ffe("FF10275", "Duplicate token pool")
	...
	MsgTokenTransferFailed          = ffe("FF10291", "Token transfer with ID '%s' failed. Please check the FireFly logs for more information")
	...
	MsgContractListenerExists       = ffe("FF10312", "A contract listener already exists in the namespace: '%s' with name: '%s'", 409)
	MsgContractMethodNotSet         = ffe("FF10313", "Method not specified on invoke contract request", 400)
	MsgContractNoMethodSignature    = ffe("FF10314", "Method signature is required if interfaceID is absent", 400)
	...
	MsgListenerNoEvent              = ffe("FF10317", "An event name and interface reference, or in-line event definition must be supplied when creating a blockchain listener", 400)
	...
	MsgNodeMissingBlockchainKey     = ffe("FF10354", "No organization signing key configured on node", 400)
	...
	MsgTokenApprovalFailed          = ffe("FF10369", "Token approval with ID '%s' failed. Please check the FireFly logs for more information")
