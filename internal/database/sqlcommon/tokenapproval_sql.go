...
func (s *SQLCommon) tokenApprovalResult(ctx context.Context, row *sql.Rows) (*fftypes.TokenApproval, error) {
	...
	if err != nil {
		return nil, i18n.WrapError(ctx, err, coremsgs.MsgDBReadErr, "tokenapproval")
	}
	return &approval, nil
}
...
func (s *SQLCommon) GetTokenApproval(ctx context.Context, connector, subject string, poolID *fftypes.UUID) (*fftypes.TokenApproval, error) {
	return s.getTokenApprovalPred(ctx, subject, sq.And{
