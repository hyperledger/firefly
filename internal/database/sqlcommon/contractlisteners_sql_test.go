...
func TestUpsertContractListenersEmpty(t *testing.T) {
	// Create a new contract listener entry
	s, _ := newMockProvider().init()
	err := s.InsertContractListeners(context.Background(), []*core.ContractListener{})
	assert.NoError(t, err)
}

func TestUpsertContractListenersFailInsert(t *testing.T) {
	// Create a new contract listener entry
	location := fftypes.JSONObject{"path": "my-api"}
	locationJson, _ := json.Marshal(location)
	sub := &core.ContractListener{
	...
	mock.ExpectExec("INSERT .*").WillReturnError(fmt.Errorf("pop"))
	mock.ExpectRollback()
	err := s.InsertContractListeners(context.Background(), []*core.ContractListener{sub})
	assert.Regexp(t, "FF00177", err)
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestUpsertContractListeners(t *testing.T) {
	s, cleanup := newSQLiteTestProvider(t)
	defer cleanup()
	...
	s.callbacks.On("UUIDCollectionNSEvent", database.CollectionContractListeners, core.ChangeEventTypeCreated, "ns", sub2.ID).Return()

	err := s.InsertContractListeners(context.Background(), []*core.ContractListener{sub, sub2})
	assert.NoError(t, err)
}
