...
func TestNodesE2EWithDB(t *testing.T) {
	...
	nodeRead, err := s.GetNode(ctx, node.Name)
	assert.NoError(t, err)
...
func TestUpsertNodeFailSelect(t *testing.T) {
	...
	mock.ExpectRollback()
	err := s.UpsertNode(context.Background(), &fftypes.Node{Name: "name1"}, true)
...
func TestGetNodeByIDSelectFail(t *testing.T) {
	s, mock := newMockProvider().init()
	mock.ExpectQuery("SELECT .*").WillReturnError(fmt.Errorf("pop"))
	_, err := s.GetNode(context.Background(), "name1")
	assert.Regexp(t, "FF10115", err)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestGetNodeByIDNotFound(t *testing.T) {
	...
	msg, err := s.GetNode(context.Background(), "name1")
	assert.NoError(t, err)
	assert.Nil(t, msg)
	assert.NoError(t, mock.ExpectationsWereMet())
}
