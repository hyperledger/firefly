...
func TestDispatchBatchSubmitBroadcastFail(t *testing.T) {
	...
	err := bm.dispatchBatch(context.Background(), &fftypes.Batch{IdentityRef: fftypes.IdentityRef{Author: "wrong", Key: "wrong"}}, []*fftypes.Bytes32{fftypes.NewRandB32()})
	assert.EqualError(t, err, "pop")
}
...
func TestSubmitTXAndUpdateDBUpdateBatchFail(t *testing.T) {
	...
	err := bm.submitTXAndUpdateDB(context.Background(), &fftypes.Batch{IdentityRef: fftypes.IdentityRef{Author: "org1", Key: "0x12345"}}, []*fftypes.Bytes32{fftypes.NewRandB32()})
...
func TestSubmitTXAndUpdateDBAddOp1Fail(t *testing.T) {
	...
	batch := &fftypes.Batch{
		IdentityRef: fftypes.IdentityRef{Author: "org1", Key: "0x12345"},
		Payload: fftypes.BatchPayload{
