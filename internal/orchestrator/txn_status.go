...
func (or *orchestrator) GetTransactionStatus(ctx context.Context, id string) (*core.TransactionStatus, error) {
	...
	case core.TransactionTypeBatchPin:
		...
		switch batches, _, err := or.database().GetBatches(ctx, or.namespace.LocalName, f.Eq("tx.id", id)); {
		case err != nil:
			return nil, err
		case len(batches) == 0:
	...
	case core.TransactionTypeTokenPool:
		...
		switch pools, _, err := or.database().GetTokenPools(ctx, or.namespace.LocalName, f.Eq("tx.id", id)); {
		case err != nil:
			return nil, err
		case len(pools) == 0:
	...
	case core.TransactionTypeTokenTransfer:
		...
		switch transfers, _, err := or.database().GetTokenTransfers(ctx, or.namespace.LocalName, f.Eq("tx.id", id)); {
		case err != nil:
			return nil, err
	...
	case core.TransactionTypeTokenApproval:
		...
		switch approvals, _, err := or.database().GetTokenApprovals(ctx, or.namespace.LocalName, f.Eq("tx.id", id)); {
		case err != nil:
			return nil, err
