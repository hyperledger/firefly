...
var (
	...
	EventAggregatorReadPageSize    RootKey = ark("event.aggregator.readPageSize")
	EventAggregatorReadPollTimeout RootKey = ark("event.aggregator.pollTimeout")
...
func Reset() {
	...
	viper.SetDefault(string(EventAggregatorReadPageSize), 100)
	viper.SetDefault(string(EventAggregatorReadPollTimeout), "30s")
