...
func newTestEthereum() (*Ethereum, func()) {
	...
	e := &Ethereum{
		...
		fireflyContract: "/instances/0x12345",
		topic:           "topic1",
...
func TestInitOldInstancePathContracts(t *testing.T) {
	...
	assert.Equal(t, e.fireflyContract, "0x71c7656ec7ab88b098defb751b7401b5f6d8976f")
}
...
func TestQueryContractOK(t *testing.T) {
	...
	result, err := e.QueryContract(context.Background(), fftypes.JSONAnyPtrBytes(locationBytes), method, input)
	assert.NoError(t, err)
