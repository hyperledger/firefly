...
func (suite *OnChainOffChainTestSuite) TestCustomContractWithMessage() {
	defer suite.testState.Done()

	received1 := e2e.WsReader(suite.testState.ws1)

	contractAddress := deployContract(suite.T(), suite.testState.stackName, "custompin/CustomPin.json")
	contractJSON := readContractJSON(suite.T(), "custompin/CustomPin.json")
	...
	_, err = suite.testState.client1.InvokeContractMethod(suite.T(), &core.ContractCallRequest{
		...
		Location:   fftypes.JSONAnyPtr(`{"address":"` + contractAddress + `"}`),
