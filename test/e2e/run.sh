...
BLOCKCHAIN_CONNECTOR=${BLOCKCHAIN_CONNECTOR:-ethconnect}
...
if [ "$CREATE_STACK" == "true" ]; then
  ...
  $CLI init --prometheus-enabled --database $DATABASE_TYPE $STACK_NAME 2 --blockchain-provider $BLOCKCHAIN_PROVIDER --blockchain-connector $BLOCKCHAIN_CONNECTOR --token-providers $TOKENS_PROVIDER --manifest ../../manifest.json $EXTRA_INIT_ARGS --sandbox-enabled=false --multiparty=$MULTIPARTY_ENABLED
  checkOk $?
