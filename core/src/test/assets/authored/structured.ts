...
describe('Asset definitions: authored - structured', async () => {
  ...
  describe('Create public asset definition', () => {
    ...
    it('Checks that the asset definition can be added', async () => {
      ...
      const getAssetDefinitionsResponse = await request(app)
        ...
        .expect(200);
      const assetDefinition = getAssetDefinitionsResponse.body.find((assetDefinition: IDBAssetDefinition) => assetDefinition.name === 'authored - structured - public');
  ...
  describe('Create private asset definition', () => {
    ...
    it('Checks that the asset definition can be added', async () => {
      ...
      const getAssetDefinitionsResponse = await request(app)
        .get('/api/v1/assets/definitions')
        .expect(200);
      const assetDefinition = getAssetDefinitionsResponse.body.find((assetDefinition: IDBAssetDefinition) => assetDefinition.name === 'authored - structured - private');
      assert.strictEqual(assetDefinition.author, '0x0000000000000000000000000000000000000001');
      assert.strictEqual(assetDefinition.confirmed, false);
