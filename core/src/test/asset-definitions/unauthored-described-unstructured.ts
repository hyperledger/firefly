...
describe('Asset definitions: unauthored - described - unstructured', async () => {
  ...
  describe('Public asset definition', async () => {
    ...
    it('Checks that the event stream notification for confirming the asset definition creation is handled', async () => {
      ...
      const data: IEventAssetDefinitionCreated = {
        ...
        name: 'unauthored - described - unstructured - public',
    ...
    it('Checks that the asset definition is confirmed', async () => {
      const getAssetDefinitionsResponse = await request(app)
        ...
        .expect(200);
      const assetDefinition = getAssetDefinitionsResponse.body.find((assetDefinition: IDBAssetDefinition) => assetDefinition.name === 'unauthored - described - unstructured - public');
      ...
      assert.strictEqual(assetDefinition.name, 'unauthored - described - unstructured - public');
  ...
  describe('Private asset definition', async () => {
    ...
    it('Checks that the event stream notification for confirming the asset definition creation is handled', async () => {
      ...
      const data: IEventAssetDefinitionCreated = {
        ...
        author: '0x0000000000000000000000000000000000000002',
        name: 'unauthored - described - unstructured - private',
        descriptionSchemaHash: sampleSchemas.assetDescription.sha256,
    ...
    it('Checks that the asset definition is confirmed', async () => {
      const getAssetDefinitionsResponse = await request(app)
        .get('/api/v1/assets/definitions')
        .expect(200);
      const assetDefinition = getAssetDefinitionsResponse.body.find((assetDefinition: IDBAssetDefinition) => assetDefinition.name === 'unauthored - described - unstructured - private');
      ...
      assert.strictEqual(assetDefinition.name, 'unauthored - described - unstructured - private');
      assert.strictEqual(assetDefinition.timestamp, timestamp);
