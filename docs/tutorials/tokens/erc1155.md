...
If you haven't set up the FireFly CLI already, please go back to the Getting Started guide and read the section on how to [Install the FireFly CLI](./firefly_cli.md).
...
- `config.amount`: The amount of tokens the other account is allowed to transfer. If `0` or not set, the approval is valid for any number. Required for `fungible` token pools.
- `config.tokenIndex`: The specific token index within the pool that the operator is allowed to transfer. If `0` or not set, the approval is valid for any token. Required for `nonfungible` token pools.
